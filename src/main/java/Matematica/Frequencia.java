...
public class Frequencia<T extends Comparable<T>> {
	...
	public double frequenciaAbsoluta(Lista<T> conjunto, T elemento) {
		...
		for(T valor : conjunto) frequencia.addValue(valor);
		return frequencia.getCount(elemento);
	}

	/**
	 * Quantidade de vezes que um elemento aparece no conjunto como valor entre 0 e
	 * 1.
	 *
	 * @param conjunto
	 * @param elemento
	 ...
	 * @return
	 */
	@SuppressWarnings("unchecked") public T moda(Lista<T> conjunto) {
		Frequency frequencia = new Frequency();
		for(T valor : conjunto) frequencia.addValue(valor);
		return (T) frequencia.getMode().get(0);
	}

	/**
	 * Quantidade de vezes que um elemento e seus antecessores aparece no conjunto,
	 * na forma de um número entre 0 e 1.
	 *
	 * @param conjunto
	 * @param elemento
	 * @return
