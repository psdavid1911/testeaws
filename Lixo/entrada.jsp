<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <script src="js/include.js"></script>
    </head>
    <body onload="CarregarPos();">
        <div class="coluna">
            <div id="menu" class="linha menu"></div>
            <div class="linha titulo">Programas!</div>
            <div class="linha subtitulo">\> David<span class="pisca">│</span></div>
            <div class="linha" style="height: 150px;"></div>
            <div class="linha">
                <a href="calculadora.jsp"><div class="projeto">Calculadora<p>Uma calculadora de expressões aritméticas sem precedencia de operadores.</p></div></a>
                <a href="analisador_sintatico.jsp"><div class="projeto">Analisador sintático<p>Um avaliador sintático que funciona com base em autômato finito.</p></div></a>
                <a href="escala_de_trabalho.jsp"><div class="projeto">Escala de trabalho<p>Resolver o problema de escalas de trabalho foi difícil até agora...</p></div></a>
                <a href="gerador_de_trabalhos.jsp"><div class="projeto">Gerador de trabalhos<p>Um assistente de criação de trabalhos acadêmicos com base na normatização da ABNT.</p></div></a>
                <a href="programa_estatistico.jsp"><div class="projeto">Programa estatístico<p>Programa que utiliza funções e métodos estatísticos.</p></div></a>
                <a href="gerenciador_de_estoque.jsp"><div class="projeto">Gerenciador de estoque</div></a>
                <a href="analisador_logico.jsp"><div class="projeto">Analisador lógico</div></a>
            </div>
            <div id="rodape" class="linha rodape"></div>
        </div>
    </body>
</html>
